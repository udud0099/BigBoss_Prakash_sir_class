...
export default async function Home() {
  ...
  async function createNote(data) {
    ...
    try {
      const newNote = await pool.query(
        "INSERT INTO ud_todo_list (task ) VALUES ($1 ) RETURNING *",
        [task]
      );
  ...
  // read
  const data = await pool.query("SELECT * FROM ud_todo_list");
  ...
  async function deleteNote(data) {
    ...
    try {
      await pool.query("DELETE FROM ud_todo_list WHERE id = $1", [id]);
